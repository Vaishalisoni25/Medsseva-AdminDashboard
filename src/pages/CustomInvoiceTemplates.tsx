...
import { CustomInvoiceTemplate, CustomTemplateType } from '../types/customFormat';
...
export const CustomInvoiceTemplatesPage: React.FC = () => {
  const navigate = useNavigate();
  const toast = useToast();

  const [templates, setTemplates] = useState<CustomInvoiceTemplate[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState('');
  const [previewTemplate, setPreviewTemplate] = useState<CustomInvoiceTemplate | null>(null);
  const [previewZoom, setPreviewZoom] = useState<number>(typeof window !== 'undefined' && window.innerWidth < 640 ? 0.45 : 0.88);
  const [deleteConfirmTemplate, setDeleteConfirmTemplate] = useState<CustomInvoiceTemplate | null>(null);
  const [actionLoading, setActionLoading] = useState<string | null>(null);

  const handleOpenPreview = (t: CustomInvoiceTemplate) => {
  ...
  const fetchTemplates = async () => {
    try {
      setLoading(true);
      const data = await customFormatService.getInvoiceTemplates();
      setTemplates(data);
    } catch (err: any) {
      toast.error('Error', err.response?.data?.error || 'Failed to load invoice templates.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSetDefault = async (t: CustomInvoiceTemplate) => {
    setActionLoading(t.id);
    try {
      await customFormatService.setDefaultInvoiceTemplate(t.id);
      toast.success('Default Updated', `“${t.name}” is now the default ${t.type.toLowerCase()} invoice template.`);
  ...
  const handleDuplicate = async (t: CustomInvoiceTemplate) => {
    setActionLoading(t.id);
    try {
      const copy = await customFormatService.duplicateInvoiceTemplate(t.id);
  ...
  const handleToggleActive = async (t: CustomInvoiceTemplate) => {
    setActionLoading(t.id);
    try {
      await customFormatService.toggleActiveInvoiceTemplate(t.id);
  ...
  const handleDelete = async () => {
    ...
    try {
      await customFormatService.deleteInvoiceTemplate(deleteConfirmTemplate.id);
      toast.success('Template Deleted', 'Invoice template removed.');
  ...
  const renderTemplateCard = (t: CustomInvoiceTemplate) => {
    const isBusy = actionLoading === t.id;
    const primaryColor = t.designSettings?.primaryColor || (t.type === 'DETAILED' ? '#005C55' : '#0F766E');
      ...
      >
        <div>
          ...
          <div className="space-y-1.5 text-xs text-muted-foreground mb-4">
            <div className="flex items-center gap-1.5">
              ...
              <span className="truncate">{t.branding?.labName || 'MedsSeva Global Healthcare'}</span>
            </div>
          ...
          >
            ...
            <div className="w-full space-y-1 mt-1">
              <div className="h-2 bg-slate-100 border border-slate-200 rounded w-full" />
              <div className="h-2 bg-slate-100 border border-slate-200 rounded w-full" />
              <div className="h-2 bg-slate-100 border border-slate-200 rounded w-full" />
            </div>
        ...
        <div className="pt-3 border-t border-border/80 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              onClick={() => navigate(`/custom-formats/invoices/${t.id}/edit`)}
    ...
    <div className="space-y-6 sm:space-y-8 pb-12">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-black text-foreground tracking-tight flex items-center gap-2">
                <span>Custom Lab Invoice Templates</span>
              </h1>
              <p className="text-xs text-muted-foreground mt-0.5">
                Customize billing headers, tax calculations, item breakdown, bank details, and QR verification
              </p>
            </div>
          </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          <p className="text-sm font-semibold text-muted-foreground">Loading invoice templates from database...</p>
        </div>
      ) : (
        <div className="space-y-8 sm:space-y-10">
          {/* 1. STANDARD INVOICE SECTION */}
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-border">
              <div>
                <div className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 rounded-full bg-teal-600 shrink-0" />
                  <h2 className="text-base sm:text-lg font-black text-foreground uppercase tracking-wide">Standard Invoice</h2>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Clean, compact laboratory billing invoice for walk-in and home collection orders
                </p>
              </div>
              ...
              >
                <Plus className="w-4 h-4" /> Add New Standard Invoice
              </button>
            </div>
            ...
            {standardTemplates.length === 0 ? (
              <div className="p-6 sm:p-8 border border-dashed border-border rounded-2xl text-center bg-muted/10 space-y-3">
                ...
                <div>
                  <h4 className="text-sm font-bold text-foreground">No Standard Invoice Templates Found</h4>
                  <p className="text-xs text-muted-foreground mt-0.5">Create your first standard invoice template</p>
                </div>
                <button
                  onClick={() => navigate('/custom-formats/invoices/new?type=STANDARD')}
                ...
                >
                  <Plus className="w-4 h-4" /> Create Standard Invoice
                </button>
              </div>
            ) : (
          ...
          {/* 2. DETAILED INVOICE SECTION */}
          <div className="space-y-4 pt-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-border">
              <div>
                <div className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 rounded-full bg-purple-600 shrink-0" />
                  <h2 className="text-base sm:text-lg font-black text-foreground uppercase tracking-wide">Detailed Invoice</h2>
                  <span className="text-xs bg-muted text-muted-foreground font-bold px-2 py-0.5 rounded-md border border-border">
                    {detailedTemplates.length} Saved
                  </span>
                </div>
                <p className="text-xs text-muted-foreground mt-0.5">
                  Comprehensive corporate tax invoice with HSN/SAC codes, itemized GST, bank details & terms
                </p>
              </div>

              <button
                onClick={() => navigate('/custom-formats/invoices/new?type=DETAILED')}
                className="w-full sm:w-auto justify-center px-4 py-2 rounded-xl bg-purple-700 hover:bg-purple-800 text-white text-xs font-black flex items-center gap-1.5 shadow-md shadow-purple-700/10 transition-all active:scale-95 shrink-0"
              >
                <Plus className="w-4 h-4" /> Add New Detailed Invoice
              </button>
            </div>
            ...
            {detailedTemplates.length === 0 ? (
              <div className="p-6 sm:p-8 border border-dashed border-border rounded-2xl text-center bg-muted/10 space-y-3">
                <Receipt className="w-8 h-8 text-muted-foreground mx-auto" />
                <div>
                  <h4 className="text-sm font-bold text-foreground">No Detailed Invoice Templates Found</h4>
                  <p className="text-xs text-muted-foreground mt-0.5">Create your first comprehensive detailed tax invoice template</p>
                </div>
                <button
                  onClick={() => navigate('/custom-formats/invoices/new?type=DETAILED')}
                  className="px-4 py-2 rounded-xl bg-purple-700 text-white text-xs font-bold hover:bg-purple-800 inline-flex items-center gap-1.5 shadow-xs"
                >
                  <Plus className="w-4 h-4" /> Create Detailed Invoice
                </button>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
            ...
            <div className="p-3.5 bg-muted/30 border border-border rounded-xl text-xs space-y-1">
              ...
              <div><strong>Type:</strong> {deleteConfirmTemplate.type} Invoice</div>
